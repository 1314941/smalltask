		...
		try {
			while (true) {
				...
				case 3:
					...
					if (type == 1) {
						// 保存文件
						// logService.saveAndAppendMatchedLog(matchedLogs);
						// mysql
				...
				case 4: {
					...
					matchTransports = tranService.readMatchedTransportFromDB();
					tranService.showMatchTransport(matchTransports);
					/*
					 * 信息保存到内存 if (matchedLogs == null || matchedLogs.size() == 0) {
					 * System.out.println("匹配的日志记录是0条！"); } else { // 输出匹配的日志信息
					 * logService.showMatchLog(matchedLogs); } if (matchedTrans == null ||
					 * matchedTrans.size() == 0) { System.out.println("匹配的物流记录是0条！"); } else { //
					 * 输出匹配的物流信息 tranService.showMatchTransport(matchedTrans); }
					 */
				}
					break;
				case 5:
